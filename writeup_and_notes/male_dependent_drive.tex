...
52-54 in][]{Burt2006} in these cases is challenging, it appears that the extent to which \emph{In} and \emph{Om} distort the second female meiotic division partially depends on the genotype of the fertilizing sperm \citep{Agulnik1993,Wu2005}.  
...
We first focus on models in which `self-promoting' alleles in
  sperm facilitate drive of like \gc{alleles} during gametogenesis in heterozygous females. 
...
These models show that sperm modifiers of female meiotic drive are
	unlikely to create a sustained genomic conflict, and therefore 
	female meiosis will have little incentive to evolve resistance to them.
We then examine models in which an allele in sperm acts to modify the efficacy of a polymorphic meiotic driver - 
	which universally reveal that sperm acting drive modifiers are favored only if the suppress drive. 
The results highlight the fact that the interests of sperm and egg  genomes' are often aligned, as both are invested in the fate of the resultant zygote (as was speculated for the \emph{In} locus \citep{Pomiankowski1993}).
...
probability  $d > 1/2$, regardless of sperm genotype (Model 1) \gc{REF}. The timing of fertilization relative to female meiosis places another constraint on $d$, for example, if fertilization (and therefore, sperm dependent drive) takes place at MII (as in mammals),
	female drive requires an uneven number of crossovers between the centromere and the drive locus, 
	so $d$ is bounded to be $<0.83$ \citep[see ][ for
        discussion]{Buckler1999}.  To depict a case
...
Models 2 and 3, above, explored the dynamics of an allele drove in females when signaled by a complementary signal in sperm.   
...
Additionally, tight linkage between female driver and sperm modifier is consistent with the nature of well characterized drive systems which are often maintained as polymorphic inversions with numerous linked modifiers \gc{CITE}.
...
The $B^+$ allele acts in sperm to increase the effectiveness of drive for both
	the $B$ and  $B^{+}$ alleles in $AB$ and $AB^{+}$ heterozygotes (see Figure \ref{Eggsperm_3_allele_cartoon}). 
...
Therefore, a sperm-acting drive facilitator alleles experience a profound disadvantage 
...
We have found no parameter range of this
	...
	invade the population (eqn. \eqref{coupling}, Appendix Model 4, Mathematical Supplement). 
...
three alleles: $A$, $B$ and $C$, corresponding to the case when the
