...
%Naively the $B^{+}$ allele might be able to increase in frequency in the
...
Sexual reproduction is a high-stakes event that determins which chromosomes are transmitted to the next generation.  
As a consequence of this intense competition for inclusion in the next generation, alleles that  gain a transmission advantage during reproduction 
	can succeed evolutionarily, even if they have bad effects on the population, 
	the mother whose offspring they will inhabbit, 
...
We find that an allele which when in sperm help distort female meiosis in its favor, %YB [too awkward / unclear?] 
	can rarely spread through a population if it bears any cost. 
Additionally, when this self-promoting driver can spread, it cannot be maintained as a protected polymorphism, and due to its positive frequency dependence 
...
This likely explains why females can delay the completion of meiosis until after fertilization 
	without exploitation by collaborations by female drivers and collaborative sperm.
...
In fact, an allele in sperm which causes the alternate allele to drive can be favored (FIG), a counterintuitive result 
	consistent with the tenuous evidence that the non-driving $ln^-$ allele is over transmitted when fertilized by $ln^+$ sperm \cite{agulnick}. 
...
\yb{DOES THIS GO ANYWHERE? }
 We speculate that other effects of sperm on female meiosis may also be tolerated by females as a way of ensuring meiotic fairness, 
 	for example in Drosophila and other taxa, sperm accessory gland proteins (ACPs) increase the female recombination rate \cite{SOMEthING}.
Presumably this is a consequence of the resolution of double strand breaks initiated by the stressful/toxic composition of ACPs. 
However, by increasing the recombination rate sperm may decrease the effectiveness of female meiotic drivers active during MI, 
	pleiotropically enhancing female fitness in a way (while decreasing it in many others, \cite{Partridge and Hurst})
