...
Therefore, an allele in sperm that facilitates meiotic drive by a genetically equivalent allele in female meiosis %(a  heteromorphic dyad), 
...
However, a severe homozygous fitness cost prevents  
	drivers from fixing  \citep{Prout1973}, 
	providing an opportunity for the evolution of
	drive suppressors (CITE - citep{XX}), corresponding well to 
...
The self-promoting driver must overcome this homozygous fitness cost simply to spread when rare.
...
\yb{When fully recessive, a self-promoting driver can only invade when $s$, the selection against the drive homozygote, is less than $\frac{2 d - 1}{4 d}$ (The tan region in Figure  \ref{Invasion_space}).} 
...
\yb{In fact, while a rare self promoting driving can only invade if $s<\frac{2 d - 1}{4 d}$, a common self-promoting driver can fix over a broader range of parameter space (i.e. if $s<\frac{2d-1}{2}$). 
This system therefore displays bistability, when $\frac{2 d - 1}{4 d}<s<\frac{2d-1}{2}$, the driver will deterministically fix when introduced at a frequency greater than $\frac{1-2d+4ds}{2s(2d-1)}$, but will be lost otherwise.}
...
This generates numerous conflicts  \cite{Burt2006} including sexual conflicts between mates \cite{Arnqvist2005}, 
	and conflicts between alleles that are overtransmitted in meiosis and the organisms they injure while doing so. 
...
For analytical solutions presented above, we assume that $F$ 
}
