...
In the standard single-locus, biallelic model of female meiotic drive, the driving allele is transmitted to the egg  in heterozygotes with probability  $d > 1/2$, regardless of sperm genotype. 
...
%\yb{ We present a general framework for this problem in equations A1-A4, and evaluate specialized cases below.}
...
Compared to the invasion criteria, parameter space allowing fixations is much broader -- 
	a common, recessive, self-promoting driver fixes if  $s$ is less than $(2d-1)/2$. 
...
Inclusion of a heterozygous fitness cost (i.e. $w_{AB}=1-h_s$)
	%(e.g. additive fitness cost) 
	further constrains the evolution of a self-promoting driver. 
This case also displays bistability -- with any heterozygous fitness cost, a rare self-promoting driver cannot spread. 
However, if $s < d - 1/2+s_h(3-2d)/2$, 
	it will fix deterministically if its 
...
%GRAHAM I HAVE REWRITTEN THIS SECTION TO REFLECT MATHS AND MY PARAMETERIZATION. WE CAN WORK ON SYNCHING OUR NOTATION
