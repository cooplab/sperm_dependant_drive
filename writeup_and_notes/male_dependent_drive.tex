...
\yb{We first focus on models in which `self-promoting' alleles in sperm facilitate drive of like genotypes during gametogenesis in heterozygous females. }
...
\yb{We then examine models in which an allele in sperm acts to modify the efficacy of a polymorphic meiotic driver - 
...
\yb{Rather, features of o\"{o}genesis  may evolve to facilitate an interaction with sperm %with female meiosis
...
probability  $d > 1/2$, regardless of sperm genotype (Model 1). To depict a case
...
\yb{We concluded with an analysis of a related model (Model 3) - in which drivers influence their transmission in females via paternal genotype, rather than sperm haplotype.}
...
The parameter space where the allele can invade but not fix is shown in white
        in Figure \yb{\ref{Invasion_space}A}. %, and it can fix in the colored regions. 
...
\yb{	a self-promoting driver specifically creates low fitness drive homozygotes 
...
However, when the distorting effect of genotype displays strong dominance in its
	...
	(see Figure S\ref{Effect_of_dominance}, and Equation \ref{polymale}).  
...
Additionally, tight linkage between female driver and sperm modifier is consistent with the nature of well characterized drive systems which are often maintained as polymorphic inversions with numerous linked modifiers (CITE).
...
We have found no parameter range of this
	...
	invade the population \yb{(Mathematical Supplement and Figure XXX)}. 
...
\yb{If a sperm-acting allele that restores fairness to female meiosis arises on
...
If this sperm-acting drive suppressor arises on 
	...
	for the drive system (see \yb{Figures XXX} and Equations \ref{A-} and \ref{unlinked}).  
...
\yb{Therefore, all two-locus models of sperm influence on female drive suggest that
