...
\subsection*{Model 1. Traditional driver}
...
Frequencies of genotype, $i$, after drive and selection are 
	...
	Mathematical approximations assuming HWE show that a rare driver will spread when $s_h <(2d-1)/(1+2d)$, and will fix when $s< d -1/2 + 3 s_h /2- d s_h$. This later inequality reduces to $s<(2d-1)/2$ when the cost of drive is fully recessive. 
	
\subsection*{Model 2. Single locus, sperm-dependent drive.}
...
When fully recessive, the change in frequency of the self-promoting driver across generations $\Delta f_B$ is exactly 
\begin{equation}
f_A f_B^2 \left( [1-F][ d(1-2 f_A s) - f_B s-1/2] - s  F \right)/\bar{w}
\label{deltadriver}
\end{equation}
where $F$ is the deviation from Hardy-Weinberg expectations (HWE). 
%For analytical solutions presented above, we assume that $F$ equals zero. 
Assuming HWE a common, recessive, self-promoting driver invades if $(2 d - 1)/(4 d)$, and fixes if
 $s<(2d-1)/2$. 
 Therefore, when \begin{equation}(2 d - 1)/(4 d)<s<(2d-1)/2\label{ineqA}\end{equation} a
...
\begin{equation} 
f^*_{B\text{ recessive}} = (1-2d+4ds)/(2s(2d-1)) \label{bistab_homozy_approx}
\end{equation}
...
When the fitness of drive heterozygotes is compromised ($s_h>0$), a rare, self promoting driver cannot invade when rare.
This results from the fact that
when the allele is rare ($f_B \ll 1$) sperm carrying it rarely
encounter heterozygous eggs ($\sim f_B^2$) but the allele still pays a
cost in heterozygous individuals. 
...
\yb{We compare this analytical approximation to exact results obtained by a simple parameter search in Supplementary Figure \ref{bistable_additive}. }
