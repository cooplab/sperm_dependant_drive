...
If a sperm-acting allele that restores fairness to female meiosis arises on
	...
	for the drive system ($AB$, see Figure and Equations XXX). 
...
The fitness of genotype, $i$ is $w_g$ independent of its sex. 
We denote its frequency in adults in the current generation is $f_g$. In the next generation of
zygotes (accounting for drive) but before selection
the frequency is $f_g'$, and in the adults after selection the
frequency is $f_g''$ and population mean fitness,  $\bar{w}=\Sigma f_g'w_g$. Genotypic fitnesses are $w_{AA} = 1$, $w_{AB} = 1-s_h$, and $w_{BB}=1-s$, and are sex-independent. 
...
models are presented in a \yb{Supplementary Worksheet}, and  our
\gc{Mathematical supplement} for our analytical approximations work.  
...
Frequencies of genotype, $i$, after drive and selection are 
	...
	Assuming HWE, a rare driver will spread when $s_h
        \lesssim(2d-1)/(1+2d)$, and will fix when $s\lesssim d -1/2 +
        3 s_h /2- d s_h$. This later inequality reduces to
        $s\lesssim(2d-1)/2$ when the cost of drive is fully
...
Figure \ref{Invasion_space} by plotting \gc{$f_B=f_{BB}+ \frac{1}{2}g_{AB}$}
over time, and to assess the analytical approximations, below. \gc{To
assess invasion or fixation criteria we iterate this system and
...
\label{deltadriver}
\end{equation}
where $F$ is the deviation from \gc{Hardy Weinberg Expectations} (HWE). 
%For analytical solutions presented above, we assume that $F$ equals zero. 
Assuming HWE \gc{($F=0$)} a common, recessive, self-promoting driver invades if $s\lesssim(2 d - 1)/(4 d)$, and fixes if
 $s\lesssim(2d-1)/2$. 
 Therefore, when 
\begin{equation}
(2 d - 1)/(4 d)  \lesssim  s  \lesssim  (2d-1)/2\label{ineqA}
\end{equation} 
...
Compared to exact results (Supplementary Figure \ref{Bistab_homozyg_cost_fig}), Equations
 \gc{\eqref{ineqA} and} \eqref{bistab_homozy_approx} offers a reasonable, but imperfect approximation. 
...
show that this approximation is reasonablely correct \gc{for small
parameter values. }
...
When the $C$ allele is tightly linked to and on a different background than the driver, 
	genotypic fitnesses equal $w_{AC}=w_{CC}=w_{AA}=1$, and $w_{BC}=1-s_h$. 
%We address two questions concerning the $C$ allele - can it invade when rare  and can it fix when common?
...
Assuming HWE and linkage equilibrium, an $F$ allele increases in frequency when 
	$-\epsilon f_A f_B (f_B s + s_h (fA - fB) ) >0$, corresponding to any level of drive suppression ($\epsilon<0$) 
	when the the fitness cost of the driver does not display over- or under-dominance. 
