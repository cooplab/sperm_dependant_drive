...
heterozygous egg ($AB$) is fertilized by a spermwith the following
male ($\Mars$) genotypes the $B$ allele will be transmitted through
female meiosis with the follow probabilites
\begin{tabular}{cccc}
$\Mars$ geno. & AA & AB  & BB\\
B transmission prob. & $\frac{1}{2}$ &  $\frac{1}{2} + h(d-\frac{1}{2})$  & $d$\\ 
\end{tabular}
where $d > 1/2$ and $h$ ($0<h<1$) acts to specify the dominance of the $B$
allele's effect on female meiotic drive through the males genotype. We
set up a set of recursions, similar to those in eqn.
...
\end{eqnarray}
in combination with the transmission probabilities given in Figure \ref{Eggsperm_3_allele_cartoon}A.
