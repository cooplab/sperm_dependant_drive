...
Because only one of four products of female meiosis is transmitted to
offspring, o\"{o}genesis is inherently susceptible to such conflicts,
as alleles can compete to be transmitted into the egg. 
%(meiotic drive).  
Genomics conflicts drive the rapid evolution of suppression mechanisms, and
	many facets of  o\"{o}genesis and gametogenesis have been 
...
Why then is female meiosis often left uncompleted until after fertilization - 
	potentially providing an opportunity for sperm alleles to
        meddle with its outcome in their own favor? %and facilitate meiotic drive? 
...
These results are consistent with current knowledge of sperm-dependent meiotic drivers 
	\yb{(loci whose distortion of female meiosis depends on sperm genotype),} 
	and suggest that the requirement of fertilization for the
        completion of female meiosis, and variation in stage of meiosis that
        fertilization occurs at may represent a mechanism employed by females 
...
These models show that such sperm-acting alleles 
	\yb{have more difficulty invading a population than do traditional meiotic drivers, 
...
We then examine models in which a \yb{novel} sperm-acting allele modifies the efficacy of a polymorphic meiotic driver. 
Such model universally reveal that sperm-acting drive modifiers are favored only if the suppress drive. 
These results highlight the fact that the interests of sperm and \yb{maternal}  genomes' are often aligned, as both are invested in the fate of the resultant zygote (as was speculated for the \emph{In} locus \citep{Pomiankowski1993}).
...
of a self-promoting meiotic driver,  we modify this standard model such 
	that the driver is only effective when fertilized by a sperm carrying that allele (see Figure \ref{Eggsperm_2_allele_cartoon} and Model 2 in the Supplementary Worksheet). 
...
For comparative purposes, we briefly present the standard drive model %as Model 1 in the Appendix, 
...
The parameter space where the allele can invade but not fix is shown in white
        in Figure \ref{Invasion_space}A. %, and it can fix in the colored regions. 
...
%We complement this single-locus approach %of an allele with pleiotropic effects in both sexes, 
%	with alternative models of two linked loci - one a female driver, 
%	and the other, a sperm allele which modifies the effect of drive upon fertilization. 
%In this model, a female meiotic driver with no sperm-dependency 
%	initially reaches drive-viability equilibrium (with two alleles
%	$A$ and $B$ are the ancestral non-driver and driver alleles). 
...
\yb{We conclude by analyzing models with alternative linkage relationship between driver and drive modifier - 
...
%We first consider a sperm-acting modifier of
%	female meiotic drive tightly linked to the $B$ background (Model 4).
%We treat this non-recombining drive/modifier haplotype as a third allele, and we label this allele as $B^+$ when it enhances drive. 
%(i.e. the non-recombining haplotype of a
...
First, %the sperm-acting drive facilitator (
...
In light of that it is interesting to consider whether the phylogenetic variation in the stage of oogenesis when fertilization takes place reflects not
	only species' different reproductive ecologies, but also different selection pressures shaping the interaction of sperm alleles with female meiosis. 
\yb{this feels weak if we don't say anything more explicit. I also think maybe we simply state predictions:
	(1) MII drivers should drive less often when fertilized by complimentary sperm. 
	(2) Drive may be more severs in interspecific crosses in which sperm have not specifically evolved to resists  a driver. 
	(3) ??? Any more specific hypotheses -- maybe making this last sentence more explicit, e.g. self-fertilizing or otherwise frequently inbreeding animals will not require fertilization for the completion of meiosis... [note - this runs counter to our empirical examples of mice and C-elegens]}
...
We highlight simple analytical results from: 
	...
	female meiotic drive (Model 3); and more complex two-locus sperm-dependent drive (Models 4-6). %changed this to minimize confusion with refs, figs, equations, etc.
