...
\begin{flushleft}
{\Large
\textbf{Sperm should evolve to make female meiosis fair.}
}
% Insert Author names, affiliations and corresponding author email.
\\
Yaniv Brandvain$^{1,\ast}$, 
Graham Coop$^{2}$, 
\\
\bf{1} Yaniv Brandvain Department of Plant Biology, University of Minnesota --€“ Twin Cities, St, Paul, MN, USA
\\
\bf{2} Graham Coop Center for Population Biology \& Department of Evolution and Ecology, University of California -- Davis. Davis, CA, USA
\\
$\ast$ E-mail: ybrandva@umn.edu
\end{flushleft}
...
probability  $d > 1/2$, regardless of sperm genotype (e.g. \cite{Ubeda2004}, and see Model 1 in the \emph{Models} for more details). 
To depict a case of a self-promoting meiotic driver,  we modify this standard model such 
	that the driver is only effective when fertilized by a sperm
        ...
        in \emph{Models} and Appendix S1). 
...
This paternal-genotype dependent model (Model 3 in \emph{Models}) requires one additional parameter, as we exchange $d$ in the sperm dependent case for $d_{het}$ and $d_{hom}$ which describe the transmission of the drive allele in a heterozygous female mating with males heterozygous and homozygous for the self-promoting drive allele, respectively.  
...
The $B^+$ allele acts in sperm to increase the effectiveness of drive for both
	the $B$ and  $B^{+}$ alleles in $AB$ and $AB^{+}$
        ...
        Model 4 in \emph{Models} and Appendix S1). 
...
We have found no parameter range of this
	...
	invade the population (Model 4 in \emph{Models}, eqn. \eqref{coupling}, and Appendix S2). 
...
Imagine a sperm-acting allele that restores fairness to female meiosis arises on
	the drive background, creating a
        third allele $B^{-}$ (Figure
          \ref{Eggsperm_3_allele_cartoon}A3, Model 4 in \emph{Models}). This new allele still
...
Transmission rules describing the outcomes of all matings in each
	model are presented in a Appendix S1. 
...
We verify these approximations with exact numerical iterations in Figures S1-S3 and Appendix S3, and detail the derivation of these approximations in our Appendix S2.  
...
After drive and random mating, genotype frequencies are 
	\begin{equation} 	f_{AA}'=f_A (f_{AA }+ f_{AB} (1 - d)), \end{equation} 
	\begin{equation} 	f_{AB}'=f_A (f_{BB} + f_{AB}d) + f_B (f_{AA} + f_{AB} (1 - d)), \text{and }\end{equation}
	\begin{equation} 	f_{BB}'= f_B (f_{AB} d + f_{BB}). \end{equation}
...
Therefore, genotype frequencies after drive are 
	\begin{equation} 	f_{AA}'=f_A^2, \end{equation} 
	\begin{equation} 	f_{AB}'=f_A f_B+f_B (f_{AA} + f_{AB}(1 - d)), \text{and } \end{equation}
	\begin{equation} 	f_{BB}'= f_B (f_{AB} d + f_{BB} ).\end{equation}
...
\label{deltadriver}
\end{equation}
where $F$ is the deviation from genotypic frequencies expected under Hardy-Weinberg.  Assuming HWE  ($F=0$) a common, recessive, self-promoting driver invades if $s\lessapprox(2 d - 1)/(4 d)$, and fixes if
 $s\lessapprox(2d-1)/2$. 
 Therefore, when 
\begin{equation}
...
In the case when female meiotic drive depends on paternal genotype,
 ...
 when mated with to $AA$, $AB$, or $BB$ males,  respectively. 
  In this model, genotype frequencies after drive, random mating, and selection are 
  	\begin{equation} f_{AA}'' =   w_{AA}\left( f_{AA} [2 f_A + f_{AB} ] + f_{AB}^2 [1 - d_{het}] \right)/(2\bar{w}) , \end{equation}
	\begin{equation} f_{AB}'' =   w_{AB}(f_{AA} [f_B + f_{AB}/2] + f_{AB} f_B + f_{BB} [ f_A - f_{AB} d_{hom}])/\bar{w}  , \text{and } \end{equation} 
	\begin{equation} f_{BB}'' =   w_{BB}(f_{AB} (f_{AB} d_{het}/2 + f_{BB} d_{hom}) + f_{BB} f_B)/\bar{w} . \end{equation}
...
More generally, even when the cost of drive is not fully recessive, the $B^-$ allele will invade and fix under all parameters sustaining a previous polymorphism at the drive locus (see Appendix S2). 
...
\end{equation}
For all values of interest ($0<s<1$, $0.5<d_0<1$), the change in frequency a rare $C$ allele is positive when it decreases drive (i.e. $\epsilon <0$, corresponding to the $A^-$ model, above), a result which holds qualitatively for a common $C$ allele, as well (Appendix S2). 
...
\newpage
\begin{figure}
\begin{center} \includegraphics[width = 1 \textwidth]{Figures/selfprom.eps} \end{center}
\caption{ {\bf{A model of single locus, male-dependent meiotic drive. }} {\bf{A.}} A visual depiction of our model of `self-promoting' driver. 
...
\caption{ {\bf{Models of a sperm-acting drive modifier tightly linked to a meiotic driver. }}
{\bf A.} 
Sperm carrying the derived allele at the modifier locus (filled squares) alters transmission  
	...
	\emph{A1:} In the standard model of drive there is no
        variation at the modifier, 
	and the driver is transmitted to the egg with probability $d_0$. 
	\emph{A2:} The modifier allele increases the transmission of the drive allele ($d_1 > d_0$), and due to their shared genetic background, also increases its drive. 
	\emph{A3 and A4:} The sperm-acting modifier acts to decrease
...
\emph{B1:} The $B^-$ allele replaces the ancestral drive haplotype, but segregates at a lower equilibrium frequency.
\emph{B2:} The $A^-$ allele replaces the ancestral non-driving haplotype, and in this case, removes the driver from the population. }
...
\subsection*{Legends for Supplementary Appendices}

\paragraph{Appendix S1: Transmission rules.} We detail the frequency of offspring genotypes produced by each possible mating for Models 1-6 in the tabs of this Excel spreadsheet.


\paragraph{Appendix S2: Analytical results.} We attach a PDF of our Mathematica spreadsheet in which we derive analytical results for models 1-6. 

\paragraph{Appendix S3: Exact approach.} We attach an R Script used for exact recursions. 

\subsection*{Supplementary Figures and Supplementary  Figure Legends}
...
\caption{  {\bf{Invasion analysis for a self-promoting female meiotic drive allele with
...
\caption{{\bf{The unstable equilibrium frequency for a self-promoting
...
\caption{{\bf{Comparing exact results to analytical approximations for the model of paternal-dependent female drive.}} Exact results of invasion analysis of an allele whose effect
...
In the parameter space below the invasion (solid) line the self-promoter
 ...
 In the final panel we show the
  ...
  ($(d_{het}-1/2)/d_{het}$) see \emph{Models} for more details.
