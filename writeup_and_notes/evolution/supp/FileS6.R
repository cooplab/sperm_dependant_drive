...
new.meiotic.table<-read.csv(file="FileS4.csv",as.is=TRUE)[,-1]
...
save(file="~/FileS5.Robj",tr,order.meiotic.table.phy,new.meiotic.table)
load(file="~/FileS5.Robj")
...
if(FALSE){
meiotic.table<-read.csv("FileS4.csv",head=FALSE,as.is=TRUE,skip=3)
